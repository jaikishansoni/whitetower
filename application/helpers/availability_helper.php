    ...
    foreach ($period as $dt) {
        ...
        if ($total_rooms > 0) {
            ...
            if (count($orders) >= $total_rooms) {
                ...
                redirect('front/bookapartments/index' . $query);
            } else {
...
function check_room_availability($check_in, $check_out, $adults, $kids, $room_type_id) {
    $query = '?date_from=' . $check_in . '&date_to=' . $check_out . '&adults=' . $adults . '&kids=' . $kids . '&room_type=';
    $CI = & get_instance();
    if ($check_in == $check_out) {
        $check_out = date('Y-m-d', strtotime($check_out . '+ 1 day'));
    ...
    $CI->db->where('id', $room_type_id);
    $CI->db->select('room1_types.*,base_price as price');
    $room_type = $CI->db->get('room1_types')->row_array();

    $CI->db->where('room1_type_id', $room_type_id);
    $CI->db->select('rooms1.*,count(room1_no) as total_rooms');
    $rooms = $CI->db->get('rooms1')->row_array();
    $total_rooms = $rooms['total_rooms'];
    ...
    foreach ($period as $dt) {
        ...
        $CI->db->where('O.room1_type_id', $room_type_id);
        $CI->db->where('R.date', $date);
        $CI->db->select('R.*,');
        $CI->db->join('room_orders O', 'O.id = R.order_id', 'LEFT');
        $orders = $CI->db->get('rel_room_orders_prices R')->result_array();
        ...
        if ($total_rooms > 0) {
            ...
            if (count($orders) >= $total_rooms) {
                ...
                $CI->session->unset_userdata('coupon_data');
                $CI->session->set_flashdata('error', "Sorry.. This Dates Between Rooms Not Available Please Try With Another Date Or Room");
                redirect('front/bookrooms/index' . $query);
            } else {
                continue; // continue loop
            }
        } else {
...
function check_room_availability_ajax($check_in, $check_out, $adults, $kids, $room_type_id) {
    $query = '?date_from=' . $check_in . '&date_to=' . $check_out . '&adults=' . $adults . '&kids=' . $kids . '&room_type=';
    $CI = & get_instance();
    if ($check_in == $check_out) {
        $check_out = date('Y-m-d', strtotime($check_out . '+ 1 day'));
...
function room1_alot($check_in, $check_out, $room_type_id) {
    //echo $check_in;echo $check_out;
    $CI = & get_instance();
    if ($check_in == $check_out) {
        $check_out = date('Y-m-d', strtotime($check_out . '+ 1 day'));
    ...
    $CI->db->where('R.room1_type_id', $room_type_id);
    $CI->db->select('R.*,F.name floor');
    $CI->db->join('floors F', 'F.id = R.floor_id', 'LEFT');
    $rooms = $CI->db->get('rooms1 R')->result();
