        ...
        <div class="row">
            ...
            <div class="panel panel-primary margin-40-y mx-auto" >
                <div class="panel-heading" >
                    <h4 class="panel-title" style="line-height: 35px;"> 
                        <?php echo lang('room_type') ?> - <?php echo @$apartment_type->title ?> - <?php echo date_convert($_GET['date_from']) ?> to <?php echo date_convert($_GET['date_to']) ?>  <button class="btn btn-primary pill px-4 py-2 pull-right" id="show_search" style="cursor:pointer;"><i class="fa fa-calendar"></i> <?php echo lang('reschedule') ?></button></h4>
                </div>
                <div class="panel-body">
                    <div id="search_div" class="margin-40-y">
                        <form method="get" class="p-5 bg-white" action="<?php echo site_url('front/bookapartments/index') ?>">
                            <div class="reservation">
                                ...
                                <div class="row form-group">
                                    <div class="col-md-6 mb-3 mb-md-0">
                                        ...
                                        <input type="time" class="form-control" name="check_in_time" placeholder="<?php echo lang('check_in'); ?>" value="<?php echo @$_GET['check_in_time'] ?>" required="">
                                    </div>
                    ...
                    <form method="post" action="<?php echo site_url('front/bookapartments/step1') ?>" class="p-5 bg-white" id="viewform">
                        <?php
                        ...
                        //$amount	=	$apartment_type->base_price * $_GET['adults'] * $nights;	//old
                        ...
                        <input type="hidden"  name="check_in_time" value="<?php echo $_GET['check_in_time'] ?>"/>
                        <input type="hidden"  name="check_out_time"   value="<?php echo $_GET['check_out_time'] ?>"/>
                                    ...
                                    <table width="100%">
                                        ...
                                        foreach ($taxes as $t) {
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo ($t->type == 'Fixed') ? $this->setting->currency_symbol : '' ?><?php echo $t->rate ?> <?php echo ($t->type == 'Percentage') ? '%' : '' ?></td>
    ...
    $(document).ready(function () {
        ...
        $('.paid_service').on('ifChecked', function (event) {
            ...
            if (id) {
                call_loader();
                $.ajax({
                    url: '<?php echo site_url('front/bookapartments/add_service_price') ?>',
                    type: 'POST',
        ...
        $('.paid_service').on('ifUnchecked', function (event) {
            ...
            if (id) {
                call_loader();
                $.ajax({
                    url: '<?php echo site_url('front/bookapartments/less_service_price') ?>',
                    type: 'POST',
                    data: {id: id, total: total, adults: adults, nights: nights},
                    success: function (result) {
