...
class Bookapartments extends Front_Controller {
    ...
    function index() {
        ...
        $data['apartment_types'] = $this->homepage_model->get_room_types_all();
        $data['taxes'] = $this->homepage_model->get_taxes();

        if (!empty($_GET['apartment_type'])) {
            $data['services'] = $this->homepage_model->get_paid_services($_GET['room_type']);
        }

        if (empty($_GET['room_type'])) {

            $this->render('bookapartments/room_types', $data);
        } else {

            check_availability($_GET['date_from'], $_GET['date_to'], $_GET['adults'], $_GET['kids'], $_GET['room_type']);

            $data['apartment_type'] = $this->homepage_model->get_apartment_type($_GET['room_type']);

            $this->render('bookapartments/view', $data);
        }
    }
    ...
    function step1() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            redirect('front/bookapartments/payment');
        } else {
            $this->session->unset_userdata('booking_data');
    ...
    function payment() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if (!empty($pay)) {
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    redirect('front/bookapartments/pay');
                }
            }
        }
        ...
        $this->render('bookapartments/payment', $data);
    }
    ...
    function pay() {
        ...
        if ($booking_data['payment_gateway'] == 1) {
            redirect('front/bookapartments/paypal');
        }
        if ($booking_data['payment_gateway'] == 2) {
            redirect('front/bookapartments/stripe');
        }
        if ($booking_data['payment_gateway'] == 3) { // Pay On Arrival
            ...
            redirect('front/bookapartments/order');
        }
        //Set variables for paypal form
        $returnURL = site_url() . '/front/bookapartments/success'; //payment success url
        $cancelURL = site_url() . '/front/bookapartments/cancel'; //payment cancel url
        $notifyURL = site_url() . '/front/bookapartments/ipn'; //ipn url
    ...
    function stripe() {
        ...
        $data['page_title'] = lang('payment');
        $this->render('bookapartments/stripe', $data);
    }
...
//            $this->session->unset_userdata('coupon_data');
            ...
            $this->render('bookapartments/order', $data);
        } else {
    ...
    function pdf($id) {
        ...
        $html = $this->load->view('bookapartments/pdf', $data, true);
    ...
    function mail_booking($order_id) {
        ...
        $html = $this->load->view('bookapartments/mail', $data, true);
