...
class Bookrooms extends Front_Controller {
    ...
    function index() {
        ...
        $data['room_types'] = $this->homepage_model->get_room1_types_all();
        ...
        if (empty($_GET['room_type'])) {
            ...
            $this->render('bookrooms/room_types', $data);
        } else {

            check_room_availability($_GET['date_from'], $_GET['date_to'], $_GET['adults'], $_GET['kids'], $_GET['room_type']);

            $data['room_type'] = $this->homepage_model->get_room1_type($_GET['room_type']);

            $this->render('bookrooms/view', $data);
        }
    }
    ...
    function step1() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $room_type = $this->homepage_model->get_room1_type($_POST['room_type']);
            ...
            $base_price = get_room_price($_POST['date_from'], $_POST['date_to'], $_POST['room_type'], $_POST['adults'], $_POST['kids']);
            ...
            redirect('front/bookrooms/payment');
        } else {
            $this->session->unset_userdata('booking_data');
    ...
    function payment() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if (!empty($pay)) {
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    $save['room1_type_id'] = $booking_data['room_type_id'];
        ...
        $data['meta_description'] = $this->setting->meta_description;
        $data['meta_keywords'] = $this->setting->meta_keywords;
        $this->render('bookrooms/payment', $data);
    }
    ...
    function pay() {
        ...
        if ($booking_data['payment_gateway'] == 1) {
            redirect('front/bookrooms/paypal');
        }
        if ($booking_data['payment_gateway'] == 2) {
            redirect('front/bookrooms/stripe');
        }
        if ($booking_data['payment_gateway'] == 3) { // Pay On Arrival
            ...
            $this->book_model->update_room_order($save, $id);

            $this->session->set_flashdata('message', "Your Booking Payment Success");
            redirect('front/bookrooms/order');
        }
        //Set variables for paypal form
        $returnURL = site_url() . '/front/bookrooms/success'; //payment success url
        $cancelURL = site_url() . '/front/bookrooms/cancel'; //payment cancel url
        $notifyURL = site_url() . '/front/bookrooms/ipn'; //ipn url
    ...
    function stripe() {
        ...
        $data['page_title'] = lang('payment');
        $this->render('bookrooms/stripe', $data);
    }
    ...
    function order() {
        ...
        if (!empty($booking_data)) {
            $data['order'] = $this->book_model->get_room_order($booking_data['order_id']);
            ...
            $this->mail_booking($booking_data['order_id']);
            $this->render('bookrooms/order', $data);
        } else {
    ...
    function pdf($id) {
        ...
        $html = $this->load->view('bookrooms/pdf', $data, true);
    ...
    function mail_booking($order_id) {
        ...
        $data['order'] = $order = $this->book_model->get_room_order($order_id);
        ...
        $html = $this->load->view('bookrooms/mail', $data, true);
