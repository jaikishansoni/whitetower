...
class Room1_types extends Admin_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model(array('room1_type_model', 'amenities_model'));
    }

    function index() {
        $admin = $this->session->userdata('admin');

        $data['page_title'] = lang('room1_types');
        $data['room1_types'] = $this->room1_type_model->get_all();
        //echo '<pre>'; print_r($data['floors']);
        $this->render_admin('room1_types/list', $data);
    }

    function view($id, $tab = false) {
        
        $data['room1_type'] = $room1_type = $this->room1_type_model->get($id);
        
        $data['page_title'] = lang('view') . " " . lang('room1_type');
        $this->render_admin('room1_types/view', $data);
    }
    ...
    function form($id = false) {
        ...
        $data['page_title'] = lang('room1_type_form');
        ...
        if ($id) {
            $data['room1_type'] = $room1_type = $this->room1_type_model->get($id);
            $amenities = $this->room1_type_model->get_amenities($id);
            ...
            if (!$room1_type) {
                $this->session->set_flashdata('error', lang('room1_type_not_found'));
                redirect('admin/room1_types');
            }

            //set values to db values
            $data['id'] = $room1_type->id;
            $data['title'] = $room1_type->title;
            $data['slug'] = $room1_type->slug;
            $data['shortcode'] = $room1_type->shortcode;
            $data['description'] = $room1_type->description;
            $data['base_occupancy'] = $room1_type->base_occupancy;
            $data['higher_occupancy'] = $room1_type->higher_occupancy;
            $data['extra_bed'] = $room1_type->extra_bed;
            $data['extra_beds'] = $room1_type->extra_beds;
            $data['kids_occupancy'] = $room1_type->kids_occupancy;
            $data['base_price'] = $room1_type->base_price;
            $data['additional_person'] = $room1_type->additional_person;
            $data['extra_bed_price'] = $room1_type->extra_bed_price;

            $data['images'] = $this->room1_type_model->get_images($id);
        }
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->render_admin('room1_types/form', $data);
        } else {
            ...
            if ($id) {
                $slug = $this->room1_type_model->validate_slug($slug, $id);
            } else {
                $slug = $this->room1_type_model->validate_slug($slug);
            }
            //echo	$slug;die; 
            //echo '<pre>'; print_r($_POST);die;
            $save['id'] = $id;
            ...
            $p_key = $this->room1_type_model->save($save);
            $amenities = $this->input->post('amenity');
            $this->room1_type_model->delete_roomtype($p_key);
            if (!empty($amenities)) {
                $i = 0;
                foreach ($amenities as $ind => $val) {
                    $save_amenity[$i]['amenity_id'] = $ind;
                    $save_amenity[$i]['room1_type_id'] = $p_key;
                    $i++;
                }
                $this->room1_type_model->save_amenity($save_amenity);
            }
            ...
            if (!empty($files)) {
                ...
                for ($i = 0; $i < $cpt; $i++) {
                    if (!empty($files['image']['name'][$i])) {
                        ...
                        $save_img['room1_type_id'] = $p_key;
                        $save_img['image'] = $file_name;
                        $this->room1_type_model->save_images($save_img);
            ...
            if ($id) {
                $this->session->set_flashdata('message', lang('room1_type_update'));
            } else {
                $this->session->set_flashdata('message', lang('room1_type_save'));
            }

            redirect('admin/room1_types/form/' . $p_key . '?show=image');
        }
    }

    function delete($id = false) {
        if ($id) {
            $room1_type = $this->room1_type_model->get($id);
            //if the customer does not exist, redirect them to the customer list with an error
            if (!$room1_type) {
                $this->session->set_flashdata('error', lang('room1_type_not_found'));
                redirect('admin/room1_types');
                ...
                $delete = $this->room1_type_model->delete($id);
                $images = $this->room1_type_model->get_images($id);
                foreach ($images as $img) {
                    $this->room1_type_model->delete_image($img->id);
                ...
                $this->session->set_flashdata('message', lang('room1_type_delete'));
                redirect('admin/room1_types');
            }
        } else {
    ...
    function updateimg() {
        ...
        $id = $_POST['id'];
        $gallery = $this->room1_type_model->get_image($id);
        ...
        $this->room1_type_model->delete_image($id);
    }
        ...
        if (!empty($files)) {
            ...
            for ($i = 0; $i < $cpt; $i++) {
                if (!empty($files['image']['name'][$i])) {
                    ...
                    //$file_name	= time().rand(1,988).'.jpg';
                    $save_img['image'] = $file_name;
                    $this->room1_type_model->update_images($save_img, $id);
    ...
    function featured() {
        //echo '<pre>'; print_r($_POST);die;
        //update set 0 to all by room1_type_id
        $value = 0;
        $this->room1_type_model->unset_featured($value, $_POST['room1_type_id']);

        //set featured
        $this->room1_type_model->set_featured($_POST['id']);
    }
