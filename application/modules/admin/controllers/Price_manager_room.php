...
class Price_manager_room extends Admin_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model(array('price_manager_room_model', 'room1_type_model'));
    }

    function index() {
        $data['page_title'] = lang('price_manager');
        $data['prices'] = $this->price_manager_room_model->get_all();
        //echo '<pre>'; print_r($data['floors']);
        $this->render_admin('price_manager_room/list', $data);
    }
    ...
    function form($id = false, $tab = false) {
        ...
        if ($id) {
            ...
            $data['price'] = $price = $this->price_manager_room_model->get($id);
            ...
            $data['id'] = $price->id;
            $data['room1_type_id'] = $price->room1_type_id;
            $data['mon'] = $price->mon;
            $data['tue'] = $price->tue;
            $data['wed'] = $price->wed;
            $data['thu'] = $price->thu;
            ...
            $data['spl_prices'] = $this->price_manager_room_model->get_special_price_rooms($data['room1_type_id']);
        }
        if (!$id) {
            $this->form_validation->set_rules('room1_type_id', 'lang:room1_type', 'trim|required');
        }
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->render_admin('price_manager_room/form', $data);
        } else {
            $save['id'] = $id;
            if ($id) {
                $save['room1_type_id'] = $data['room1_type_id'];
            } else {
                $save['room1_type_id'] = $this->input->post('room1_type_id');
            }
            ...
            $p_key = $this->price_manager_room_model->save($save);
            if (!empty($_POST['title'])) {
                $date = explode('/', $_POST['date']);
                if ($id) {
                    $save_spl['room1_type_id'] = $data['room1_type_id'];
                } else {
                    $save_spl['room1_type_id'] = $this->input->post('room1_type_id');
                }
                ...
                $this->price_manager_room_model->save_special_price_room($save_spl);
            }
            if ($id) {
            ...
            redirect('admin/price_manager_room');
        }
    }

    function delete($id = false) {
        if ($id) {
            $price_manager = $this->price_manager_room_model->get($id);
            ...
            if (!$price_manager) {
                ...
                $delete = $this->price_manager_room_model->delete($id);
    ...
    function delete_spl_price($id, $room1_type_id) {
        if ($id) {
            $delete = $this->price_manager_room_model->delete_spl_price($id);

            $this->session->set_flashdata('message', lang('spl_price_delete'));
            redirect('admin/price_manager_room/form/' . $room1_type_id . '/1');
        }
    }

    function get_room1_type_data() {
        $room1_type_id = $_POST['id'];
        $price = $this->price_manager_room_model->get_room1_type_price($room1_type_id);
        ...
        if (!empty($price)) {
            ...
            $price = $this->room1_type_model->get($room1_type_id);
    ...
    function check_start_date() {
        ...
        $price = $this->price_manager_room_model->check_daterange();
        //echo '<pre>'; print_r($price);die;
        if (empty($price)) {
            echo 0;
            die;
