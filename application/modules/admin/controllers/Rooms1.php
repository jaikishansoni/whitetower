...
class Rooms1 extends Admin_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model(array('room1_model', 'floor_model', 'room1_type_model', 'amenities_model', 'housekeeping_model', 'housekeping_status_model', 'employee_model', 'booking_model'));
    }
    ...
    function index() {
        ...
        $data['page_title'] = lang('rooms1');
        $data['booked_Rooms1_today'] = $this->room1_model->get_booked_room();

        $data['floors'] = $this->floor_model->get_all();
        
        $data['room1_types'] = $this->room1_type_model->get_all();
        
        $data['rooms1'] = $this->room1_model->get_all();
        
        $data['states'] = $this->room1_model->get_states();
        
        $this->render_admin('Rooms1/list', $data);
    }

    function check_room1_number() {
        if (!empty($_POST['id'])) {
            $id = $_POST['id'];
            $room = $this->room1_model->get_by_room_no($_POST['value'], $id);
            if (!empty($room)) {
                echo 1;
                exit;
            }
    ...
    function housekeeping($id) {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if ($this->form_validation->run() == true) {
                ...
                redirect('admin/Rooms1/housekeeping/' . $save['room_id']);
            }
        }
        ...
        $this->render_admin('Rooms1/housekeeping', $data);
    }
    ...
    function view($id, $tab = false) {
        ...
        $data['room_type'] = $room_type = $this->room1_type_model->get($id);
    ...
    function form($id = false) {
        ...
        $data['floors'] = $this->floor_model->get_all();
        $data['room1_types'] = $this->room1_type_model->get_all();
        $data['page_title'] = lang('room1_form');
        ...
        if ($id) {
            $data['room'] = $room = $this->room1_model->get($id);
            ...
            if (!$room) {
                ...
                redirect('admin/Rooms1');
            }

            //set values to db values
            $data['id'] = $room->id;
            $data['room1_no'] = $room->room1_no;
            $data['floor_id'] = $room->floor_id;
            $data['room1_type_id'] = $room->room1_type_id;
        }

        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            //echo '<pre>'; print_r($_POST);die;
            $rn = $this->input->post('room1_number');
            $floor_id = $this->input->post('floor_id');
            $room1_type_id = $this->input->post('room1_type_id');
            ...
            if (!empty($rn)) {
                ...
                foreach ($rn as $new) {
                    ...
                    $room = $this->room1_model->get_by_room_no($new, $id);

                    if (empty($room)) {
                        $save['id'] = $id;
                        $save['room1_no'] = $new;
                        $save['floor_id'] = $floor_id;
                        $save['room1_type_id'] = $room1_type_id[$i];
                        $p_key = $this->room1_model->save($save);
                    }
                    $i++;
                }
            }
        ...
        $this->render_admin('Rooms1/form', $data);
    }

    function delete($id = false) {
        if ($id) {
            $room = $this->room1_model->get($id);
            //if the customer does not exist, redirect them to the customer list with an error
            if (!$room) {
                $this->session->set_flashdata('error', lang('room_not_found'));
                redirect('admin/Rooms1');
                ...
                $delete = $this->room1_model->delete($id);
