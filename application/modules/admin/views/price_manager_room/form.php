...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="<?php echo site_url('admin/price_manager_room') ?>"> <?php echo lang('price_manager_room') ?> </a></li>
        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-body">
                    ...
                    <form method="post" action="<?php echo site_url('admin/price_manager_room/form/' . $id); ?>" enctype="multipart/form-data">	
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-4">
                                    <label><?php echo lang('room1_type') ?></label>
                                    <select name="room1_type_id" class="form-control" id="room1_type_id" <?php echo ($id) ? 'disabled="disabled"' : ''; ?> >
                                        <option value="">--<?php echo lang('select_room1_type') ?>--</option>
                                        <?php
                                        foreach ($room1_types as $rt) {
                                            $sel = "";
                                            if (set_select('room1_type_id', $rt->id))
                                                $sel = "selected='selected'";
                                            if ($room1_type_id == $rt->id)
                                                $sel = "selected='selected'";
                                            ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="btn-group" style="float:right">
                                                            ...
                                                            <a class="btn btn-danger" href="<?php echo site_url('admin/price_manager_room/delete_spl_price/' . $new->id . '/' . $id); ?>" onclick="return areyousure(this);"><i class="fa fa-trash"></i> </a>
                                                        </div>
                                                    </td>
                                                </tr>
                                                <?php $i++;
...
<?php } ?>
                                                                ...
                                                                timePicker: true,
                                                                        ...
                                                                        $(document).on('change', '#room1_type_id', function () {
                                                                    //alert(12);
                                                                    vch = $(this).val();
                                                                    if (vch) {
                                                                        call_loader();
                                                                        $.ajax({
                                                                            url: '<?php echo site_url('admin/price_manager_room/get_room1_type_data') ?>',
                                                                            type: 'POST',
                                                                            data: {id: vch},
...
<?php if (!empty($room1_type_id)) { ?>
                                                                        var room1_type_id = '<?php echo $room1_type_id ?>';
                                                                        var id = '<?php echo $id ?>';
<?php } else { ?>
                                                                        var room1_type_id = $('#room1_type_id').val();
                                                                        var id = '';
                                                                        if (id == '') {
...
<?php } ?>
                                                                    if (id) {
                                                                        call_loader();
                                                                        $.ajax({
                                                                            url: '<?php echo site_url('admin/price_manager_room/check_start_date') ?>',
                                                                            type: 'POST',
                                                                            data: {room1_type_id: room1_type_id, start_date: sd, end_date: ed, id: id},
                                                                            success: function (result) {
                                                                                remove_loader();
                                                                                if (result == 1) {
